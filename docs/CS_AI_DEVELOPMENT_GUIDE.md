...
*   **로그 정책**: 상세 내용은 [LOGGING_POLICY.md](./LOGGING_POLICY.md)를 참고하세요.
...
*   **DB 테이블**: 상세 내역은 [DATABASE_DESIGN_GUIDE.md](./DATABASE_DESIGN_GUIDE.md)를 참고하세요.
...
*   **처리 흐름**:
    ...
    5.  **성능 및 안정성 최적화** (2026-03-18):
        *   **비동기 논블로킹**: `WebClient`를 이용한 통신으로 서블릿 스레드 점유 최소화.
        *   **타임아웃 설정**: AI 응답 지연에 대비해 60초 타임아웃 적용 (ReadTimeout 방지).
        *   **재시도 전략 (Retry)**: 503(Service Unavailable) 에러 발생 시 **Exponential Backoff**(최대 3회, 2초부터 지수 증가)를 적용하여 일시적인 API 장애 대응.
    6.  **로그 시스템 고도화**:
        *   `mega-cs-ai.log`로 AI 추론 및 CS 연동 로그를 완전 분리.
        *   `logback-spring.xml` 내 패키지 경로(`led.mega.cs`) 기준으로 자동 필터링.
    7.  **기타 안정성**:
        *   `OpenAiAutoConfiguration` 제외 설정을 통해 불필요한 자동 설정으로 인한 부팅 에러(API Key 미설정 등) 방지.
### 5.4 SSE(`CS_EVENT` 이벤트)를 통해 관리자에게 실시간 알림
*   **AI 설정**: `led.mega.cs.config.CsAiConfig` - `ChatClient` 빈 등록 완료

### 5.5 수집 데이터 관리 및 AI 답변 생성 (2026-03-18)
*   **기능 개요**: 레드마인/메일 등에서 유입된 데이터를 관리하고, 과거 이력을 기반으로 AI의 추천 답변을 받을 수 있는 기능.
*   **상세 구현**:
    *   **AI 모범답변 생성**: 상세 팝업에서 버튼 클릭 시 `CsBotService.generateAiSolution`을 호출하여 RAG 기반 최적 답변 제안.
    *   **AI Suggestion 저장**: AI가 제안한 답변은 `ai_suggestion` 컬럼에 별도로 기록되어 실제 상담사 답변과 대조 가능.
    *   **학습 데이터 선별**: `status`를 `PROCESSED`로 변경한 건들만 AI가 다음 질문에 대한 참고 자료로 활용함.
*   **컨트롤러**: `CsInboundApiController` (CRUD 및 AI 솔루션 엔드포인트)

### 5.6 UI/UX 및 성능 최적화 (2026-03-18)
*   **페이징(Pagination)**: 상담 내역 및 수집 데이터 목록에 15개 단위 페이징 적용 (서버 사이드 처리).
*   **최신순 정렬**: 모든 목록(상담, 수집, FAQ)과 검색 결과에 `DESC`(최근 데이터 우선) 정렬 강제 적용.
*   **대시보드 통합**: 각 탭 간 유기적인 데이터 로딩 및 상태(Stats) 실시간 동기화.
*   **검색 편의성**: 검색어와 상관없이 항상 최신 정보를 먼저 보여주어 업무 우선순위 확보.

### 5.7 시뮬레이터 컨트롤러 고도화 (2026-03-18)
...
*   **주요 API**:
    ...
    | `GET`  | `/api/cs/inbound` | 수집 데이터 목록 조회 (페이징 지원) |
    | `GET`  | `/api/cs/inbound/{id}/ai-solution` | AI 모범답변 생성 및 기록 |
    | `PUT`  | `/api/cs/inbound/{id}` | 수집 데이터 처리 상태 및 답변 업데이트 |

---

## 6. 향후 확장성
*   **Redmine 연동**: 문의 유입 시 자동으로 Redmine Issue를 생성하는 웹훅 연동.
*   **Vector DB 도입**: 데이터 급증 시 단순 텍스트 검색 대신 벡터 기반 정밀 검색 도입.
*   **통계 리포트**: `CsReportService`를 통한 업무 능률 분석 리포트 생성 기능 추가 예정.
