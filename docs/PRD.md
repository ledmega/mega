...
* **동적 스케줄러 재등록 (P1)**: 60초마다 서버에서 `monitoring_config` 목록을 Pull하여 신규/변경/삭제된 서비스 모니터링 작업을 자동으로 반영합니다.
* **로그 키워드 Tail 모니터링 (P2)**: 파일 오프셋을 추적하여 새로 추가된 라인에서만 키워드 CSV를 매칭하고, 감지 시 Exception 이벤트로 즉시 전송합니다.
...
* **서비스 관리 CRUD API**: `MonitoringConfig` 엔티티 기반으로 서비스별 모니터링 설정(경로, 로그, 수집 항목, 인터벌 등)을 등록/수정/삭제/토글하는 REST API를 제공합니다. (`GET /api/configs/agent/{id}/active` 포함)
...
* **UTC→KST 시간 변환**: Agent(Ubuntu, UTC)가 전송하는 `collectedAt`은 타임존 없는 UTC 시각입니다. 차트 X축 레이블 및 메트릭 목록에서 `new Date(collectedAt + 'Z')` 처리로 브라우저 로컬(KST)로 자동 변환하여 표시합니다.
* **서비스 관리 UI**: `monitoring_config` 기반 서비스 등록/수정/삭제/토글 화면(`/services`)과 통계 카드, 필터바, 모달 폼을 제공합니다.
...
3. **[P0] ✅ 서비스 관리 웹 UI 화면** (Thymeleaf 기반 CRUD 폼 + 필터 + 통계카드)
4. **[P1] ✅ Agent Config 동적 Pull API** (`GET /api/configs/agent/{agentId}/active`)
5. **[P1] ✅ Agent 동적 스케줄러 재등록 구현** (60초마다 신규/변경/삭제 자동 반영)
6. **[P2] ✅ 로그 키워드 Tail 모니터링 구현** (파일 오프셋 추적, 키워드 CSV 매칭)
